...
class MPR():
    ...
    def get_mpr(
        ...
        total,current_idx, patient_origin, new_st_origin, new_ed_origin, center, center_origin = self.get_total_and_currentIdx_by_matrix(rebuild_spacing, slab_thickness, azimuth_type, matrix, img_shape, spacing, st_origin)
        if azimuth_type == 1:
            physical_idx = center[2]
            # 根据 slab_thickness 和 physical_idx 获取 mip 区间
            tmp1 = [new_st_origin[0], new_st_origin[1], physical_idx - 0.5 * slab_thickness]
            tmp2 = [new_st_origin[0], new_st_origin[1], physical_idx + 0.5 * slab_thickness]
            #  左面不够 0.5 * 重建厚度
            left_bounding = [new_st_origin[0], new_st_origin[1], new_st_origin[2] + slab_thickness]
            right_bounding = [new_st_origin[0], new_st_origin[1], new_ed_origin[2] - slab_thickness]
            min_idx, max_idx = self.get_rebuild_range(img, tmp1, tmp2, new_st_origin, new_ed_origin, new_spacing, rotate_m, left_bounding, right_bounding, physical_idx, slab_thickness, azimuth_idx=2)
        elif azimuth_type == 2:
            physical_idx = center[1]
            print(f'*****physical_idx: {physical_idx}')
            left_bounding = [new_st_origin[0], new_st_origin[1] + slab_thickness, new_st_origin[2]]
            right_bounding = [new_st_origin[0], new_ed_origin[1] - slab_thickness, new_st_origin[2]]
            tmp1 = [new_st_origin[0], physical_idx - 0.5 * slab_thickness, new_st_origin[2]]
            tmp2 = [new_st_origin[0], physical_idx + 0.5 * slab_thickness, new_st_origin[2]]
            min_idx, max_idx = self.get_rebuild_range(img, tmp1, tmp2, new_st_origin, new_ed_origin, new_spacing, rotate_m, left_bounding, right_bounding, physical_idx, slab_thickness, azimuth_idx=1)
        elif azimuth_type == 3:
            # 失 X
            physical_idx = center[0]
            left_bounding = [new_st_origin[0] + slab_thickness, new_st_origin[2], new_st_origin[1]]
            right_bounding = [new_ed_origin[0] - slab_thickness, new_st_origin[2], new_st_origin[1]]
            tmp1 = [physical_idx - 0.5 * slab_thickness, new_st_origin[2], new_st_origin[1]]
            tmp2 = [physical_idx + 0.5 * slab_thickness, new_st_origin[2], new_st_origin[1]]
            print(f'********tmp1: {tmp1}, tmp2: {tmp2}, right_bounding: {right_bounding}')
            min_idx, max_idx = self.get_rebuild_range(img, tmp1,tmp2,  new_st_origin, new_ed_origin, spacing, rotate_m, left_bounding, right_bounding, physical_idx, slab_thickness, azimuth_idx=0)
        else:
    ...
    def get_rebuild_range(self,img, tmp1, tmp2, new_st_origin, new_ed_origin, new_spacing, rotate_m, left_bounding,right_bounding, physical_idx, slab_thickness, azimuth_idx=2):
        """"""
