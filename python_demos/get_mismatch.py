...
mask_path = "/media/tx-deepocean/Data/DICOMS/demos/1572"
...
class Mismatch:
    ...
    def get_mismatch(
        ...
        for dcm_idx in range(self.cbf_array.shape[0]):
            rows, cols = self.cbf_array.shape[1:]
            tmip_gray_array = tmip_array[dcm_idx, :, :]
            tmip_rgb_array = gray2rgb_array(tmip_gray_array, 100, 50)
            [i, j, loc] = origin

            loc = loc + (self.depth - 1 - dcm_idx) * self.spacing[2]
            ...
            # 获取某层
            cbf_gray_array = self.cbf_array[dcm_idx, :, :]
            cbv_gray_array = self.cbv_array[dcm_idx, :, :]
            tmax_gray_array = self.tmax_array[dcm_idx, :, :]

            # # 计算倾斜角
            angle = (math.atan2(y1 - y2, x1 - x2)) / math.pi * 180
            center = (512 / 2, 512 / 2)

            (cbf_lesion_mask, cbf_high, cbf_mid, cbf_low,
            ...
            slice_view_info[dcm_idx] = {}
            rcbf_lesion_mask.append(cbf_lesion_mask)
            cbf_high = cbf_high if cbf_high else 0
            cbf_low = cbf_low if cbf_low else 0
            cbf_mid = (cbf_mid) if cbf_mid else 0
            slice_view_info[dcm_idx].update(
                {"rcbf<0.4": cbf_high, "rcbf<0.3": cbf_mid, "rcbf<0.2": cbf_low}
            )
            # 获取 rcbv 视图
            (cbv_lesion_mask, cbv_high, cbv_mid, cbv_low,) = self.get_rcbv_result(
                tmip_rgb_array,
                cbv_gray_array,
                center,
                angle,
            ...
            (
                ...
                tmip_rgb_array, tmax_gray_array, index
            )
            slice_view_info[dcm_idx].update(
            ...
            tmax_mid = len(tmax_mid[0] if tmax_mid else 0)
            tmax_low = len(tmax_low[0] if tmax_low else 0)

            slice_view_info[dcm_idx].update(
                {"tmax>10": tmax_high, "tmax>8": tmax_mid, "tmax>6": tmax_low}
            )
            brain_gray_array = self.brain_area_array[dcm_idx, :, :]
            # 获取 脑干 mask
    ...
    def get_data_result(
        ...
        for lesion in cc_unique:
            ...
            for area_label in brain_list:
                ...
                for dcm_idx in range(slice_ids[lesion][0], slice_ids[lesion][1] + 1):
                    res1 += slice_view_info[dcm_idx]["low_perfusion"]
                    res2 += slice_view_info[dcm_idx]["core_infarct"]
                    res_cbf_high += slice_view_info[dcm_idx]["rcbf<0.4"]
                    res_cbf_low += slice_view_info[dcm_idx]["rcbf<0.2"]

                    res_cbv_high += slice_view_info[dcm_idx]["rcbv<0.45"]
                    res_cbv_mid += slice_view_info[dcm_idx]["rcbv<0.4"]
                    res_cbv_low += slice_view_info[dcm_idx]["rcbv<0.35"]

                    res_tmax_high += slice_view_info[dcm_idx]["tmax>10"]
                    res_tmax_mid += slice_view_info[dcm_idx]["tmax>8"]
            # Mismatch
    ...
    def _get_alone_brain_tendency(
        ...
        low_perfusion = round(
            np.sum(low_perfusion_mask[self.brain_area_array == label_origin])
            * self.voxel,
            1,
        )
        core_penumbra = round(
            np.sum(core_infarct_mask[self.brain_area_array == label_origin])
            * self.voxel,
            1,
        )
    ...
    def get_mismatch_result(
        self,
        tmip_rgb_array: np.ndarray,
        cbf_gray_array: np.ndarray,
        tmax_gray_array: np.ndarray,
        cbv_gray_array: np.ndarray,
        mtt_gray_array: np.ndarray,
        ttp_gray_array: np.ndarray,
        center: float,
        angle: float,
        index: int,
        rows,
        ...
        tmip_rotated_image = cv2.warpAffine(
            src=rgb_array, M=rotate_matrix, dsize=cbf_gray_array.shape
        )
        # cbf 沿旋转矩阵旋转angle
        cbf_rotated_image = cv2.warpAffine(
            src=cbf_gray_array, M=rotate_matrix, dsize=(self.rows, self.cols)
        )
        # tmax 沿旋转矩阵旋转angle
        tmax_rotated_image = cv2.warpAffine(
            src=tmax_gray_array, M=rotate_matrix, dsize=(self.rows, self.cols)
        )

        # 镜像翻转
        mirror = np.fliplr(cbf_rotated_image)
        ...
        tmax_setting = 6
        tmax_error_arr = np.where(tmax_rotated_image > tmax_setting)
        mask1[tmax_error_arr] = 255
        mis_threshold2_errs = np.where(tmax_gray_array > tmax_setting)

        cbf_setting = 30
        l_mean = cbf_rotated_image[:, :256].mean()
        ...
        cbf_error_arr = np.where((0 < temp_cbf) & condition)
        mask2[cbf_error_arr] = 255
        mis_threshold1_errs = cbf_error_arr

        kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (5, 5))
        sum_B = get_cc(mask1, kernel)
        sum_B = cv2.morphologyEx(sum_B, cv2.MORPH_CLOSE, kernel, iterations=CLOSED_ITER)
        tmip_rotated_image[sum_B > 0] = color_map["#FFFF00"]
        # 低灌注信息目前固定 Tmax>6
        single_low_mask[tmax_gray_array > 6] = 1
        thresh2 = cv2.bitwise_and(mask2, sum_B)
        thresh2 = cv2.morphologyEx(
            thresh2, cv2.MORPH_CLOSE, kernel, iterations=CLOSED_ITER
        )
        tmip_rotated_image[thresh2 > 0] = color_map["#FF5959"]
        # 梗死信息目前固定 tmax>10
        # core_infarct_errors = np.where((0 < temp_cbf) & (temp_cbf < 0.3))
        core_infarct_errors = np.where(tmax_gray_array > 10)
        single_core_mask[core_infarct_errors] = 1
        core_infarct_errors = (
            core_infarct_errors[0].shape[0] if core_infarct_errors[0].size != 0 else 0
        )
        # 获取病灶 mask 
        rotated_lesion_mask = self._get_new_lesion_mask(
            cbf_gray_array,
            cbv_gray_array,
            tmax_gray_array,
            mtt_gray_array,
        ...
        lesion_mask = cv2.warpAffine(
            src=rotated_lesion_mask, M=rotate_matrix, dsize=(rows, cols)
        )
        origin_image = bgr_to_rgb(origin_image)
        # 根据阈值更新梗死 低灌注 计算方式
        low_perfusion_errors = np.where(tmax_gray_array > 6)
        low_perfusion_errors = (
            low_perfusion_errors[0].shape[0] if low_perfusion_errors[0].size != 0 else 0
        )

        self.result["Mismatch_" + str(index)] = origin_image
        return lesion_mask.tolist(), single_low_mask,single_core_mask,core_infarct_errors,low_perfusion_errors,mis_threshold1_errs, mis_threshold2_errs,
    ...
    def get_rcbf_result(
        ...
        cbf_high, cbf_mid, cbf_low = [], [], []
        rgb_array = tmip_rgb_array.copy()
        single_core_mask = np.zeros((cbf_gray_array.shape), np.uint8)

        # tmip 沿旋转矩阵旋转angle
        rotate_matrix = cv2.getRotationMatrix2D(center=center, angle=angle+90, scale=0.75)
        tmip_rotated_image = cv2.warpAffine(
        ...
        for rcbf_threshold in rCBF:
            ...
            if cbf_setting == 40:
                cbf_high = error_arr[0].shape[0] if error_arr[0].size != 0 else 0
                mask1[error_arr] = 255
            if cbf_setting == 30:
                mask2[error_arr] = 255
                cbf_mid = error_arr[0].shape[0] if error_arr[0].size != 0 else 0
            if cbf_setting == 20:
                cbf_low = error_arr[0].shape[0] if error_arr[0].size != 0 else 0
                mask3[error_arr] = 255
        kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (5, 5))
        sum_B = get_cc(mask1, kernel)
        ...
        self.result["rCBF_Mismatch_" + str(index)] = origin_image
    ...
    def get_rcbv_result(
        ...
        self.result["rCBV_" + str(index)] = origin_image
        # cv2.imwrite("./result_rcbv.jpg", origin_image)
        return lesion_mask.tolist(), cbv_high, cbv_mid, cbv_low

    def get_tmax_result(self, tmip_rgb_array, tmax_gray_array, index):
        rgb_array = tmip_rgb_array.copy()
        ...
        self.result["TmaxMismatch_" + str(index)] = rgb_array
        return (
            lesion_mask.tolist(),
            tmax_high,
            tmax_mid,
    ...
    def _get_new_lesion_mask(
        ...
        lesion_mask = np.zeros((cbf_gray_array.shape), np.uint8)
        rotated_lesion_mask = cv2.warpAffine(
            src=lesion_mask, M=rotate_matrix, dsize=(rows, cols)
        )
        cbf_rotated_image = cv2.warpAffine(
            src=cbf_gray_array, M=rotate_matrix, dsize=(rows, cols)
        )
        cbf_mir = np.fliplr(cbf_rotated_image)
        cbf_setting = 30
        l_mean = cbf_rotated_image[:, :256].mean()
        r_mean = cbf_mir[:, :256].mean()
        temp_cbf = np.divide(
            cbf_rotated_image,
            cbf_mir,
            out=np.zeros_like(cbf_rotated_image),
        ...
        error_arr = np.where((0 < temp_cbf) & condition)
        rotated_lesion_mask[error_arr] = 1
        if not error_arr:
            # cbv 沿旋转矩阵旋转angle
            cbv_rotated_image = cv2.warpAffine(
                src=cbv_gray_array, M=rotate_matrix, dsize=(rows, cols)
            )
            cbv_mir = np.fliplr(cbv_rotated_image)
            cbv_setting = self.lesion_threshold["rCBV"]

            l_mean = cbv_rotated_image[:, :256].mean()
            r_mean = cbv_mir[:, :256].mean()
            temp_cbv = np.divide(
                cbv_rotated_image,
                cbv_mir,
                out=np.zeros_like(cbv_rotated_image),
                where=cbv_mir > 0,
            )
            condition = (
                (temp_cbv < (cbv_setting / 100))
                if l_mean <= r_mean
                else (temp_cbv > 1 / (cbv_setting / 100))
            )
            error_arr = np.where((0 < temp_cbv) & condition)
            rotated_lesion_mask[error_arr] = 1
        return rotated_lesion_mask
...
def main():
    ...
    brain_array = sitk.GetArrayFromImage(
        sitk.ReadImage(
            "/media/tx-deepocean/Data/DICOMS/demos/1572/brain_area_mask.nii.gz"
        )
    )

    tmax_array = sitk.GetArrayFromImage(
        sitk.ReadImage("/media/tx-deepocean/Data/DICOMS/demos/1572/TMAX.nii.gz")
    )
    tmip_img = sitk.ReadImage(
        "/media/tx-deepocean/Data/DICOMS/demos/1572/TMIP_NO_SKULL.nii.gz"
    )
    ...
    mtt_array = sitk.GetArrayFromImage(
        sitk.ReadImage("/media/tx-deepocean/Data/DICOMS/demos/1572/MTT.nii.gz")
    )
    ttp_array = sitk.GetArrayFromImage(
        sitk.ReadImage("/media/tx-deepocean/Data/DICOMS/demos/1572/TTP.nii.gz")
    )
    centerline = {
        "point": [6.186867887533687,-93.65708726750445,-539.6293130325345],
        "vector": [-0.99585155190342, -0.034110867634475024, -0.08435718867277055],
    }
    t_3 = time.time()
    depth = tmip_img.GetDepth()
    mis = Mismatch(
