...
from vtkmodules.vtkIOGeometry import vtkBYUReader, vtkOBJReader, vtkSTLReader
...
def get_program_parameters():
    ...
    parser.add_argument(
        "tgt_fn", help="The polydata target file name, e.g. greatWhite.stl."
    )
...
def main():
    ...
    trnf = vtkTransform()
    if (
        Path(src_fn).name == "Grey_Nurse_Shark.stl"
        and Path(tgt_fn).name == "greatWhite.stl"
    ):
        trnf.RotateY(90)
    ...
    print("Distances:")
    print(
        "  Before aligning:                        {:0.5f}".format(
            distance_before_align
        )
    )
    print(
        "  Aligning using oriented bounding boxes: {:0.5f}".format(
            distance_before_align
        )
    )
    print(
        "  Aligning using IterativeClosestPoint:   {:0.5f}".format(distance_after_icp)
    )
    ...
    source_actor.GetProperty().SetDiffuseColor(colors.GetColor3d("White"))
    ...
    target_actor.GetProperty().SetDiffuseColor(colors.GetColor3d("Tomato"))
...
def best_bounding_box(
    axis, target, source, target_landmarks, source_landmarks, best_distance, best_points
):
    distance = vtkHausdorffDistancePointSetFilter()
